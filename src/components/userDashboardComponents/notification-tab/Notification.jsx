...
const Notification = () => {
  ...
  return (
    <div className="bg-[#C0C0C0] relative rounded-full p-[8px] mr-[10px] cursor-pointer">
      <img
        src={icon}
        alt=""
        className="h-[20px]"
      ...
      />
      {tdata?.length > 0 && (
        <p className="rounded-full z-50 h-4 w-4 bg-egreen text-[0.7rem] grid place-items-center absolute top-0 right-0 font-[200]">
          {tdata?.length}
        </p>
      )}
  ...
  return (
    <div className="font-[100] border-bgrey p-2  hover:border-[#fff] border-[1px]">
      <p className="text-[0.9rem]">
        {data?.userName} has requested to rent your{" "}
        <strong>{data?.carName}</strong>
      </p>
      <span>
        <p>
          Projected Earning:{" "}
          <strong>GHS {data?.rentalPrice?.toFixed(2)}</strong>
        </p>
        <span className="flex gap-1 justify-end">
          <button
            className="bg-egreen p-1"
            onClick={() => {
